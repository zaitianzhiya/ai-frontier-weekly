...
class RealSearchCollector(BaseCollector):
    ...
    def collect(self) -> list[EventRecord]:
        ...
        if self.src_type == "rss":
            ...
            # Tier 2: lighter DDG search (fewer keywords, fewer items)
            q = f"{self.source_name} {self.keywords[0]}"
            results = _fetch_ddg(q, max_results=min(self.max_items, 3))
            if not results:
                print(f"    [{self.source_key}] no results — skipped (no skeleton)")
                return []
        else:
